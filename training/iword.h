...
class iword
{
 ...
 public:
  ...
  cost getFertCst(unsigned int fert) const;
  unsigned int getFertSize() const;
  string print() const;
};
...
#ifndef VOCCORRESPONDENCE
#define VOCCORRESPONDENCE
class vocCorrespondence {
  vector<wordIndex> vocToRedVoc;  // vector of reduced index given the vocabulary index
 public:
  void insert ( wordIndex vocInd, wordIndex redVocInd ) {
    if (vocToRedVoc.size() < vocInd){
      cerr<<"ERROR: vocToRedVoc size is smaller than voc index\n";
    }else if (vocToRedVoc.size()==vocInd){
      vocToRedVoc.push_back(redVocInd);
    }else{
      if (vocToRedVoc.at(vocInd) != redVocInd){
	cerr<<"ERROR: vocabulary correspondence mismatch!\n";
      }
    }
  }
  wordIndex reduce (wordIndex vocInd) const {
    return vocToRedVoc.at(vocInd);
  }
};
#endif
